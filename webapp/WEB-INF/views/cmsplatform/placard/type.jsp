...
<script type="text/javascript">
	...
	function addOrupdateActType(type, id, name) {
		if (type == 'add') {
			...
			$("#acttypeId").html('请输入类型名称：');
		} else {
			updateId = id;
			$('#addOrupdateActType').dialog('setTitle', '修改--' + name);
			$("#acttypeId").html('请输入新的类型名称：');
		}
	...
	function acttypeAddorUpdate() {
		...
		if (name.replace(/\s/gi, '').length < 1) {
			divfloat(flag + '类型名不能为空', 250, 40);
			$('#acttypeName').focus();
			return false;
		}
		var lenname = name.replace(/[^\x00-\xff]/g, "**").length;
		if (lenname > 10) {
			divfloat('类型名过长', 250, 40);
			$('#acttypeName').focus();
			return;
		}
		...
		$.ajax({
			type : "post",
			async : false,
			url : global_ctxPath + '/addOrupdatePlacardType',
			data : {
				"id" : updateId,
				"type" : name
			},
	...
	function acttypeDeletes(ary) {
		$.ajax({
			type : 'post',//传输方式
			cache : false,
			url : global_ctxPath + '/deletePlacardtypes',//路径
			dataType : 'json',
			data : {
				"ary" : ary
			},
