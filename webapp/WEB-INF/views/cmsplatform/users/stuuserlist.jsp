...
var current = '${pages}';
				...
				.datagrid(
						{
							...
							columns : [ [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											if (value == "2") {
												return operate = "<img src='images/green-flag.png' title='认证通过' width='20px' height='20px'>";
											} else if (value == "1") {
												return operate = "<img src='images/red-flag.png' title='认证失败' width='20px' height='20px'>";
											} else {
												return operate = "<img src='images/yellow-flag.png' title='未认证' width='20px' height='20px'>";
											}
										}
									},
									{
										...
										formatter : function(value, row, index) {
											...
											operate = '<img src="images/view-flag.png" title="查看" width="20px" style="cursor:pointer;" height="20px" onclick="activitiesShow('
												+ usersid + ')">';
											console.log("value:" + value);
											if(value == '0' || value == null) {
												validate = '<img src="images/ok-flag.png" title="验证通过" width="20px" style="cursor:pointer;" height="20px" onclick="exchange(' + usersid + ',1)">';
												validate = validate + '<img src="images/no-flag.png" title="验证拒绝" width="20px" style="cursor:pointer;" height="20px" onclick="exchange(' + usersid + ',2)">';
											}
											else if(value == '1') {
												validate = '<img src="images/no-flag.png" title="验证拒绝" width="20px" style="cursor:pointer;" height="20px" onclick="exchange(' + usersid + ',2)">';
											}
											else if(value == '2') {
												validate = '<img src="images/ok-flag.png" title="验证通过" width="20px" style="cursor:pointer;" height="20px" onclick="exchange(' + usersid + ',1)">';
											}
	...
	function doSearch() {
		$('#tables').datagrid("load", {
			...
			name : $("#name").val(),
			mobile : $("#mobile").val(),
			validate : $("#validate").combobox("getValue")
		});
	}
...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div
			...
			</select> <br /> <span>姓名：</span> <input id="name" class="textbox"
			...
			<span>认证状态：</span> <select id="status" class="easyui-combobox"
				name="status" style="width:150px;" data-options="editable:false">
				<option value="-1">所有状态</option>
				<option value="0">未认证</option>
				<option value="2">认证通过</option>
				<option value="1">认证失败</option>
			</select>
			<span>验证状态：</span> <select id="validate" class="easyui-combobox"
				name="status" style="width:150px;" data-options="editable:false">
				<option value="-1">所有状态</option>
				<option value="0">未认证</option>
			...
			<table cellspacing="0" cellpadding="0" border="0" style="width:100%">
				<tr>
					<td align="left"><a href="#" onclick="updatestoyes()"
						...
						plain="false">认证通过</a>&nbsp;&nbsp;<a href="#"
						onclick="updatestono()" class="easyui-linkbutton"
						data-options="iconCls:'icon-no'" plain="false">认证拒绝</a></td>
				</tr>
			</table>
		</div>
	</div>
