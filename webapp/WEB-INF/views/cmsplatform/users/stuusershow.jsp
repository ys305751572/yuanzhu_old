...
<script type="text/javascript">
	...
	//跳转到列表页面
	function backlist() {
		window.location.href = global_ctxPath + '/stuuserlist?pages=' + ${stuuser.pageNumber };
	}
	...
	function getStuuserById(id){
		$.ajax({
			type : "post",
			async : false,
			url : global_ctxPath + '/stuuserShow2?ary='+id,
			dataType : "json",
			...
			<table class="ttab" height="100%" width="80%" border="0"
				...
				<tr>
					...
					<td colspan="2" style="text-align: center;"><c:if
							test="${stuuser.status==0 or stuuser.status==3}">
						...
						<c:if test="${stuuser.status==1}">
							<input type="button" class="easyui-linkbutton"
								style="padding: 1px 5px 1px 5px;"
								data-options="iconCls:'icon-ok'"
								onclick="activitiesUpdateToYes('${stuuser.id }')" value="认证通过" />
						</c:if>
						</td>
				</tr>
				<tr>
